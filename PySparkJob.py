...
sc = SparkContext(appName='covid-job')
sc.setLogLevel('WARN')
spark = SparkSession(sc)
 
#Read streaming data from Kafka into Pyspark dataframe
dfCSV=spark.readStream.format('kafka').option('kafka.bootstrap.servers','localhost:9092').option('subscribe', 'covid_data_csv').option("failOnDataLoss","false").option('startingOffsets', 'earliest').load().selectExpr("CAST(value AS STRING)")
dfCSV.printSchema()
...
'Country_code',
'Country_name',
'Country_total_deaths','Extracted_timestamp')).alias('value')).writeStream.format("kafka").outputMode("complete").option("failOnDataLoss","false").option('checkpointLocation','/home/ubuntu/checkpoint_out').option("kafka.bootstrap.servers","<IP Adress>.us-east-2.compute.internal:9092").option("topic", "covid_out").start().awaitTermination()
